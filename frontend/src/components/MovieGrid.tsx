...
export const MovieGrid: React.FC<MovieGridProps> = ({ movies, onBookSeats }) => {
  ...
  const genres = ['All', 'Action', 'Sci-Fi', 'Drama', 'Animation', 'Horror', 'Premiere Rush'];
    ...
    return movies
      .filter((m) => {
        ...
        const matchesGenre = 
          ...
          selectedGenre === 'Premiere Rush' ? (m.badge === 'HOT RUSH' || m.badge === 'PREMIERE') :
    ...
    <section className="space-y-6">
      ...
      <div className="glass-panel p-5 rounded-2xl border border-white/10 space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full bg-dark-800 text-white pl-10 pr-10 py-2.5 rounded-xl border border-gray-700 text-xs focus:border-brand-500 focus:outline-none transition"
            />
            {searchQuery && (
              <button 
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end text-xs">
            ...
            <select
              ...
              className="bg-dark-800 text-gray-200 border border-gray-700 rounded-xl px-3 py-2 text-xs focus:border-brand-500 focus:outline-none"
            >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
          <span className="text-gray-500 font-bold uppercase tracking-wider text-[10px] shrink-0 mr-1">
            Genre:
          </span>
          {genres.map((g) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-xl font-bold shrink-0 transition ${
                  isActive
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 px-1">
        <span className="font-semibold text-gray-300">
          Showing <strong className="text-white">{filteredMovies.length}</strong> Blockbuster Movies
        </span>
