  ...
  try {
    ...
    const seatRes = await client.query(
      `SELECT s.id FROM seats s 
       ...
       WHERE st.id = $1 AND s.seat_code = $2`,
    ...
    await client.query(
      `INSERT INTO bookings (booking_ref, showtime_id, seat_id, user_id, amount, status)
       VALUES ($1, $2, $3, $4, $5, 'PENDING')
       ON CONFLICT (booking_ref) DO NOTHING`,
      [bookingRef, showtimeId, seatId, userId, amount]
    );
  ...
  try {
    ...
    if (res.rows.length > 0) {
      ...
      if (expiredRefs.length > 0) {
        await pool.query(
          `UPDATE bookings SET status = 'EXPIRED' WHERE booking_ref = ANY($1) AND status = 'PENDING'`,
          [expiredRefs]
        );
      }
    }
...
export async function initiatePayment(
  ...
  try {
    ...
    await pool.query(
      `UPDATE bookings SET payment_id = $1, user_phone = $2, updated_at = NOW() WHERE booking_ref = $3`,
      [payment_id, userPhone, bookingRef]
    );
    ...
    await pool.query(
      `UPDATE bookings SET user_phone = $1, updated_at = NOW() WHERE booking_ref = $2`,
      [userPhone, bookingRef]
    );
  ...
  try {
    ...
    if (event_id) {
      const existingCb = await client.query(
        `SELECT id FROM gateway_callbacks WHERE event_id = $1`,
        [event_id]
      );
      ...
      await client.query(
        `INSERT INTO gateway_callbacks (event_id, booking_ref, payment_id, status, amount, payload)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (event_id) DO NOTHING`,
        [event_id, booking_ref, payment_id, status, amount, JSON.stringify(payload)]
      );
    }
    ...
    if (status === 'SUCCEEDED') {
      // Update Booking
      await client.query(
        `UPDATE bookings SET status = 'CONFIRMED', payment_id = $1, updated_at = NOW() WHERE booking_ref = $2`,
        [payment_id || booking.payment_id, booking_ref]
      );

      // Transition seat status to BOOKED permanently
      await client.query(
        `UPDATE showtime_seats SET status = 'BOOKED', updated_at = NOW() WHERE showtime_id = $1 AND seat_id = $2`,
        [booking.showtime_id, booking.seat_id]
      );
    } else if (status === 'FAILED' || status === 'REFUNDED') {
      // Update Booking
      await client.query(
        `UPDATE bookings SET status = 'FAILED', payment_id = $1, updated_at = NOW() WHERE booking_ref = $2`,
        [payment_id || booking.payment_id, booking_ref]
      );
